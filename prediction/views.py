...
#    fig.show()
        train=dist_ulpdf[1:67]
        test=dist_ulpdf[67:]
        if pal == 'Unit Linked Pensions': pdq=(5,0,2)
        elif pal == 'Unit Linked Life': pdq=(7,0,4)
        elif pal == 'Annuity Payments (APOLLO)': pdq=(1,0,1)
        elif pal == 'Climate (Conventional Life)': pdq=(7,0,0)
        elif pal == 'HPS CKB': pdq=(7,0,6)
#
        print('Application= ', pal)
        print('pdq= ', pdq)    
#
        from statsmodels.tsa.arima_model import ARIMA
        final_model=ARIMA(dist_ulpdf,order=pdq).fit()
        prediction=final_model.predict(len(dist_ulpdf),len(dist_ulpdf)+6)
        predtest=final_model.predict((len(train)+1),(len(dist_ulpdf)-1))
...
#
        testacc = pd.DataFrame(test).reset_index()
        testacc.columns =['Month', 'Incident_Counts']
        testaccdt = testacc[testacc['Month'] > '2020-12-01']
        predacc = pd.DataFrame(predtest).reset_index()
        predacc.columns =['Month', 'Incident_Counts']
        predacc['Incident_Counts'] = predacc['Incident_Counts'].round(0)
        predaccdt = predacc[predacc['Month'] > '2020-12-01']
#
#plotly and cufflinks in offine mode
...
#
        fig = go.Figure()
        fig.add_trace(go.Bar(x=testaccdt['Month'],
            y=testaccdt['Incident_Counts'],
            name='Actual',
            marker_color='rgb(55, 83, 109)'
        ))
        fig.add_trace(go.Bar(
            x=predaccdt['Month'],
            y=predaccdt['Incident_Counts'],
            name='Prediction',
            marker_color='rgb(26, 118, 255)'
        ))
#       
        fig1 = fig.update_layout(barmode='group',
                                 title="Accuracy Check",
                                 xaxis_title="Month",
                                 yaxis_title="Incident Counts",
                                 xaxis_tickangle=-45)
        plot_div = py.plot(fig1, include_plotlyjs=False, output_type='div')
#        print ('pred_cnt.json')
...
#       fig.savefig('static/images/barchart.png')
        return render(request, 'Dashboard.html', {'plot': plot_div})
